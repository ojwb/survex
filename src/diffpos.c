...
/* Quick and dirty program to compare two SURVEX .pos files */
...
1996.05.06 fixed STRING() macro to corret Syntax: message
*/
...
int main( int argc, char *argv[] ) {
 ...
 if (argc!=3) {
  ...
  if (argc!=4 || *p) {
   ...
   printf(" where <threshold> is the max. permitted change along any axis"
          " in metres\n"
...
static void diff_pos( FILE *fh1, FILE *fh2, double threshold ) {
 ...
 while (!feof(fh1) && !feof(fh2)) {
  if (fRead1) {
   if (!read_line(fh1,&x1,&y1,&z1,id1))
    continue;
   fRead1=0;
  }
  if (fRead2) {
   if (!read_line(fh2,&x2,&y2,&z2,id2))
    continue;
   fRead2=0;
  }
  cmp=strcmp(id1,id2);
