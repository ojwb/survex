...
#if (OS==MSDOS)
# define really_plot(S,X,Y) outtextxy((X)+(unsigned)xcMac/2,(Y)+(unsigned)ycMac/2,(S))
#endif
...
void init_map( void ) {
  int i;
  width=(unsigned)xcMac/4;
  height=(unsigned)ycMac/4;
...
void fancy_label( char *label, int x, int y ) {
   ...
   if (!fAllNames) {
      ...
      if (X+len>=width) len=width-X; /* or just 'return;' to right clip */
      for( l=len-1 ; l>=0 ; l-- )
         if (map[Y][X+l])
            return;
      l=Y-2;
